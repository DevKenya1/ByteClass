...
$initials = strtoupper(substr($_SESSION['full_name'] ?? 'L', 0, 1));
?>
<aside id="sidebar" class="fixed left-0 top-0 h-full transition-all duration-300 z-40 flex flex-col"
  ...
  <nav class="flex-1 py-3 overflow-y-auto px-2">
    <?php
    $links = [
      ['/lecturer/dashboard.php', 'layout-dashboard', 'Dashboard'],
      ['/lecturer/courses.php',   'book-marked',      'My courses'],
      ['/lecturer/lessons.php',   'book-open',        'Lessons & quizzes'],
      ['/lecturer/quizzes.php',   'help-circle',      'Quizzes'], // ✅ New link added here
      ['/lecturer/sessions.php',  'calendar',         'Class sessions'],
      ['/lecturer/students.php',  'users',            'My students'],
      ['/lecturer/reviews.php',   'star',             'Performance reviews'],
      ['/lecturer/payslips.php',  'dollar-sign',      'Pay slips'],
      ['/lecturer/hr.php',        'briefcase',        'HR & forms'],
      ['/lecturer/community.php', 'message-square',   'Community'],
      ['/lecturer/support.php',   'headphones',       'Support'],
    ];
  ...
  <div style="border-top:1px solid rgba(255,255,255,0.1);padding:14px 16px;flex-shrink:0;">
    <div class="flex items-center gap-3">
      ...
      <?php else: ?>
      <div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#06B6D4,#4F46E5);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px;flex-shrink:0;">
        <?= $initials ?>
      </div>
      <?php endif; ?>
      <div class="sidebar-label overflow-hidden">
        ...
        <p style="color:#9ca3af;font-size:11px;line-height:1.3;">Lecturer</p>
      </div>
    </div>
  </div>
</aside>